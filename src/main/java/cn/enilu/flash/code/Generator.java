...
public class Generator {
	...
	public static void main(String[] args) throws Exception {
		...
		options.addOption("v", "views", true,
		...
		try {
			...
			if (commandLine.hasOption("x")) {
				excludePattern = Pattern.compile(commandLine.getOptionValue("x"), Pattern.CASE_INSENSITIVE);
			}
...
//
		...
		Loader loader = (Loader) Mirror
				.me(Lang.loadClassQuite("cn.enilu.flash.code." + Strings.upperFirst(_loader) + "DescLoader")).born();
		Map<String, TableDescriptor> tables = loader.loadTables(ioc, basePackageName, baseUri, servicePackageName,
				repositoryPackageName, modelPackageName);
		...
		for (Map.Entry<String, TableDescriptor> entry : tables.entrySet()) {
			...
			for (String type : new String[] { "model", "repository", "service", "controller", "view" }) {
				...
				if (type.equals("view")) {
					...
					File file = new File(basePath + codeConfig.getModel(type) + File.separator + outputDir,
	...
	private static void generateViews(String basePath, CodeConfig codeConfig, boolean force, TableDescriptor table,
		...
		for (String view : pages) {
			...
			File file = new File(basePath + codeConfig.getViewModel() + "/src/main/webapp/WEB-INF/view/"
					+ table.getViewBasePath() + "/" + view + ".html");
		...
		File indexFile = new File(basePath + codeConfig.getViewModel() + "/src/main/webapp/static/modular/"
				+ table.getViewBasePath() + "/" + table.getEntityNameLowerFirstChar() + ".js");
		File infoFile = new File(basePath + codeConfig.getViewModel() + "/src/main/webapp/static/modular/"
				+ table.getViewBasePath() + "/" + table.getEntityNameLowerFirstChar() + "_info.js");
