...
public class TableDescLoader extends Loader {
	...
	public Map<String, TableDescriptor> loadTables(Ioc ioc, String basePackageName, String baseUri,
			String servPackageName, String repositoryPackageName, String modPackageName) throws SQLException {
		...
		Sql tableSchemaSql = Sqls
				.create("select * from INFORMATION_SCHEMA.COLUMNS where TABLE_SCHEMA = '" + database + "'");
		...
		for (Map<String, Object> columnInfo : columns) {
			...
			if ("opAt".equals(column.columnName) || "opBy".equals(column.columnName)
					|| "delFlag".equals(column.columnName)) {
				continue;
			}
			...
			if (table == null) {
				table = new TableDescriptor(tableName, null, basePackageName, baseUri, servPackageName,
						repositoryPackageName, modPackageName);
		...
		Sql infomationSchemaSql = Sqls
				.create("select * from INFORMATION_SCHEMA.TABLES where TABLE_SCHEMA = '" + database + "'");
